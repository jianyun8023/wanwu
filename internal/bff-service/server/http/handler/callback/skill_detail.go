...
//	@Success		200			{object}	response.Response{data=response.SkillDetailForWorkflow}
//	@Router			/callback/v1/skill/detail [get]
func GetSkillDetail(ctx *gin.Context) {
	resp, err := service.GetSkillDetailForWorkflow(ctx, ctx.Query("skillId"), ctx.Query("skillType"))
