...
| --------------- | -------- | ------ | ------------------ |
| uuid            | 是       | string | 对话流唯一ID        |
| conversation_id | 是       | string | 需要删除的会话ID    |
...
| code      | int    | 状态码，0表示成功，非0表示错误                               |
| msg       | string | 提示信息，用于提供操作结果或错误的具体描述                   |
| data      | object | 响应数据主体，删除成功时为null                               |

### 调用示例

```shell
curl -k --location --request DELETE 'http://localhost:8081/service/api/openapi/v1/chatflow/conversation' \
--header 'Content-Type: application/json' \
--header 'Accept: application/json' \
--header 'Authorization: Bearer <API Key>' \
--data '{
...
| -------------------- | ------------------------------------------------------------ |
| 方法                 | Http                                                         |
| 请求URL              | 按照应用实际API根地址，例如：<br />`http://localhost:8081/service/api/openapi/v1/chatflow/conversation/list` |
| 字符编码             | UTF-8                                                        |
| 请求类型             | POST                                                         |
| 鉴权方式(header参数) | `Authorization: Bearer {API Key}`                            |
| 请求格式             | `Content-Type: application/json`                             |
...
```shell
curl -k --location --request POST 'http://localhost:8081/service/api/openapi/v1/chatflow/conversation/list' \
--header 'Content-Type: application/json' \
--header 'Accept: application/json' \
--header 'Authorization: Bearer <API Key>' \
--data '{
...
```json
{
    ...
    "data": {
        "conversations": [
            ...
            {
                "conversation_id": "7574260908419973121",
